...
{\color{blue}
	...
	\textbf{Issuer emit}: $\{H,r,s\}$
	
	\textbf{Holder} can chose a secret key tSk each time and \textbf{present}:
	$$\{H,r,s',m,t,tSg,tPk\}$$
	Where $s'=s+sign(tSk,H:r)$.\\
	Where $t$ can contain expiration date-time and the intended verifier.\\
	
	\textbf{Verifier checks}:
	\begin{itemize}
		\item t, time and session informations
		\item t signature
		\item signature s' with the publick key: $rPk,aPk,tPk$
		\item revocation status of $s'$
	\end{itemize}	
	
	\textbf{Security considerations}. The schema is secure because a evil verifier cannot recostruct the holder information $\{H,r,s\}$ necessary for reply attack. Furthermore the 3 attributes of presented credentials $t,tSg,tpk$ cannot be modified or removed because $tPk$ is necessary for $s'$ signature verification.
}
%{\color{red}
%	[per noi e basta]
